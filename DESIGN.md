...
| `--color-border-subtle` | `neutral-300` | `border-border-subtle` | Default dividers, card borders |
...
| `--color-ai-surface` | `ai-50` | `bg-ai-surface` | AI reasoning panel background |
| `--color-ai-border` | `ai-500` | `border-ai-border` | AI panel accent border/top-stripe |
...
'PERCENT'}` on an *unbound* text style's `lineHeight`, but once a variable is bound to that field
