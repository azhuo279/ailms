...
| E–H | AI + decision | Render map, cluster pins | **No-geodata fallback — [SHIPPED, see PRD FR-57 / §5.1.1]:** null-coordinate exceptions are not dropped; they surface via a map-corner "N exceptions not shown on map" indicator that links back to and snaps the feed to them. The feed and map now behave as one linked instrument (PRD FR-53–FR-56): severity-encoded pins, cluster count badge colored by worst tier, "being handled" ring on delegated/escalated exceptions, hover preview popover, cluster-pin click filters the feed, and a persistent map inset in the detail state. | Exception Feed (map mode) | `map_rendered`, `cluster_expanded` | Map view has a non-visual equivalent always available — never map-only (NFR-16/17), upheld by the off-map indicator |
