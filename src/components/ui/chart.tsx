...
export function Chart({
  ...
  if (isLoading) {
    return (
      <div
        className={cn("flex items-center justify-center rounded-lg border border-border-subtle bg-surface-raised", className)}
        style={{ height }}
        role="status"
  ...
  if (isEmpty) {
    return (
      <div className={cn("rounded-lg border border-border-subtle bg-surface-raised", className)} style={{ height }}>
        <EmptyState title="No data" description={emptyMessage} className="h-full justify-center py-0" />
      </div>
    );
  }

  return (
    <div className={cn("rounded-lg border border-border-subtle bg-surface-raised p-4", className)}>
        ...
        {type === "bar" ? (
          <BarChart data={data} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
            ...
            <YAxis
              ...
              width={36}
            />
            <Tooltip content={<ChartTooltip series={series} />} cursor={{ fill: "var(--color-option-hover)" }} />
            {series.map((s, i) => (
              <Bar key={s.key} dataKey={s.key} name={s.label} fill={resolveColor(s, i)} radius={[4, 4, 0, 0]} maxBarSize={40} />
            ))}
          </BarChart>
        ) : type === "area" ? (
          <AreaChart data={data} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
            <defs>
              {series.map((s, i) => (
                <linearGradient key={s.key} id={`${gradientId}-${s.key}`} x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor={resolveColor(s, i)} stopOpacity={0.28} />
                  <stop offset="95%" stopColor={resolveColor(s, i)} stopOpacity={0.02} />
                </linearGradient>
              ))}
            </defs>
            <CartesianGrid stroke="var(--color-border-subtle)" vertical={false} />
            ...
            <YAxis
              ...
              width={36}
            />
            <Tooltip content={<ChartTooltip series={series} />} cursor={{ stroke: "var(--color-border-strong)" }} />
            {series.map((s, i) => (
              <Area
                key={s.key}
                type="monotone"
                ...
                dot={false}
                activeDot={{ r: 4, strokeWidth: 2, stroke: "var(--color-surface-raised)" }}
              />
            ))}
          </AreaChart>
        ) : (
...
function ChartTooltip({ active, label, payload }: ChartTooltipProps) {
  ...
  return (
    <div className="rounded-md border border-border-subtle bg-surface-overlay px-3 py-2 text-body-s shadow-lg">
      {label ? <p className="mb-1 font-medium text-fg-primary">{label}</p> : null}
      <div className="flex flex-col gap-0.5">
        {payload.map((entry, i) => (
          <div key={i} className="flex items-center gap-2">
            ...
            <span className="ml-auto font-medium tabular-nums text-fg-primary">{entry.value}</span>
          </div>
        ))}
      </div>
    </div>
    ...
    <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1.5 border-t border-border-subtle pt-3">
      {series.map((s, i) => (
        <div key={s.key} className="flex items-center gap-1.5">
          <span className="size-2.5 shrink-0 rounded-full" style={{ backgroundColor: resolveColor(s, i) }} aria-hidden="true" />
...
export function ChartFrame({ title, description, actions, children, className }: ChartFrameProps) {
  return (
    <div className={cn("flex flex-col gap-3", className)}>
      <div className="flex items-start justify-between gap-3">
        <div className="flex flex-col gap-0.5">
          <h3 className="text-heading-l font-semibold text-fg-primary">{title}</h3>
